        ...
        results.append(
            {
                ...
                "power_law_deviation": (
                    (current_price - power_law_results["fair_value"])
                    / power_law_results["fair_value"]
                )
                * 100,
            }
        )
...
df["signal"] = df["composite_score"].apply(
    lambda x: "buy" if x > 0.25 else ("sell" if x < -0.15 else "hold")
)

# REALISTIC ACCURACY: Bitcoin often moves ±5-10% weekly, so Hold allows up to ±10%
df["correct"] = (
    ((df["signal"] == "buy") & (df["price_change_pct"] > 0))
    | ((df["signal"] == "sell") & (df["price_change_pct"] < 0))
    | ((df["signal"] == "hold") & (df["price_change_pct"].abs() < 10))
...
print("\nSignal Distribution:")
print(
    f"  • Buy signals:  {(df['signal'] == 'buy').sum()} ({(df['signal'] == 'buy').sum() / len(df) * 100:.1f}%)"
)
print(
    f"  • Hold signals: {(df['signal'] == 'hold').sum()} ({(df['signal'] == 'hold').sum() / len(df) * 100:.1f}%)"
)
print(
    f"  • Sell signals: {(df['signal'] == 'sell').sum()} ({(df['signal'] == 'sell').sum() / len(df) * 100:.1f}%)"
)
...
print(f"\n{'=' * 80}")
print("Run 'python3 plot_backtest.py' to visualize results")
print(f"{'=' * 80}")
