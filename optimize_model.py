...
current_weights = {"rsi": 0.20, "ma": 0.25, "pl": 0.25, "macd": 0.15, "sentiment": 0.15}

# Test variations
test_weights = [
    {"name": "Current", "rsi": 0.20, "ma": 0.25, "pl": 0.25, "macd": 0.15, "sentiment": 0.15},
    {
        "name": "Power Law Focus",
        "rsi": 0.15,
        "ma": 0.20,
        "pl": 0.35,
        "macd": 0.15,
...
for weights in test_weights:
    ...
    df["signal"] = df["new_composite"].apply(
        lambda x: (
            "buy" if x > best_buy_threshold else ("sell" if x < best_sell_threshold else "hold")
        )
    )
    ...
    print(f"\n  {weights['name']}:")
    print(
        f"    Weights: RSI={weights['rsi']:.2f}, MA={weights['ma']:.2f}, PL={weights['pl']:.2f}, MACD={weights['macd']:.2f}, Sent={weights['sentiment']:.2f}"
    )
...
print("   - Current ±2% is too strict for Bitcoin's volatility")

print("\n3. Consider reweighting factors:")
print("   - Test different weight combinations above")
print("   - Focus on factors with highest correlation")
