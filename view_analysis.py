...
print("-" * 100)
print(
    f"  RSI:       {factors['rsi']:+.3f} × {weights['rsi']:.2f} = {factors['rsi'] * weights['rsi']:+.3f}"
)
print(
    f"  MA:        {factors['moving_averages']:+.3f} × {weights['moving_averages']:.2f} = {factors['moving_averages'] * weights['moving_averages']:+.3f}"
)
print(
    f"  Power Law: {factors['power_law']:+.3f} × {weights['power_law']:.2f} = {factors['power_law'] * weights['power_law']:+.3f}"
)
print(
    f"  MACD:      {factors['macd']:+.3f} × {weights['macd']:.2f} = {factors['macd'] * weights['macd']:+.3f}"
)
print(
    f"  Sentiment: {factors['sentiment']:+.3f} × {weights['sentiment']:.2f} = {factors['sentiment'] * weights['sentiment']:+.3f}"
)
print("  " + "-" * 50)
